...
// review-bridge (local companion that runs headless Opus 4.8 to apply fixes)
...
function scanDs(root: HTMLElement): DsIssue[] {
  ...
  // Raw <button> — DS Button required
  root.querySelectorAll('button').forEach(el => {
    // DS Button renders a <button>; only flag ones with NO class (hand-rolled).
    if (!el.className || (typeof el.className === 'string' && el.className.trim() === '')) {
      push('raw-button', 'Unstyled <button> — use DS Button with variant + size', el);
    }
  });

  // Inline-style hardcoded colors + banned tokens
  root.querySelectorAll<HTMLElement>('[style]').forEach(el => {
    const s = el.getAttribute('style') || '';
    if (HEX_RE.test(s) || RGB_RE.test(s)) {
      push('hardcoded-color', 'Hex/rgb in inline style — use var(--token)', el);
    }
    if (s.includes('color-mix(in oklch')) {
      push('banned-color-mix', 'color-mix(in oklch …) selection state is banned', el);
    }
  });

  // Sub-12px text (computed) — DS floor is 12px
  root.querySelectorAll<HTMLElement>('*').forEach(el => {
    if (!el.textContent || !el.textContent.trim()) return;
    if (el.children.length > 0) return; // leaf text only
    const fs = parseFloat(getComputedStyle(el).fontSize);
    if (fs && fs < 11.5) {
      push('sub-12px-text', `Text at ${fs.toFixed(1)}px — DS minimum is 12px`, el);
    }
  });
  ...
  root.querySelectorAll<HTMLElement>('[class]').forEach(el => {
    ...
    if (/\buppercase\b/.test(c) && /\btracking-wide\b/.test(c)) {
      push('banned-uppercase-tracking', 'uppercase + tracking-wide label is banned', el);
    }
    if (/\bpy-20\b/.test(c) && /\btext-center\b/.test(c)) {
      push('banned-empty-hero', 'py-20 text-center empty-state hero is banned', el);
    }
  });
...
function DevReviewHUDInner({ product }: { product?: string }) {
  ...
  useEffect(() => {
    ...
    const ping = async () => {
      ...
      try {
        ...
        if (!stop) setBridgeUp(r.ok);
      } catch { if (!stop) setBridgeUp(false); }
  ...
  // Send every detected issue to the bridge; stream Opus 4.8's progress live.
        ...
        >
            ...
            <div style={{ padding: '7px 10px', borderBottom: '1px solid var(--border)', background: 'var(--card)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--foreground)' }}>
                  {fixing ? `Opus 4.8 fixing ${Math.min(doneN + 1, fixTotal)}/${fixTotal}` : `Done — ${fixTotal} reviewed`}
                </span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderBottom: '1px solid var(--border)', background: 'var(--muted)' }}>
            {bridgeUp ? (
              ...
              >
                <span>{fixing ? '⠋' : '✦'}</span>
                {fixing ? 'Opus 4.8 fixing…' : allClear ? 'Nothing to fix' : `Fix page (${a11yTotal + dsTotal}) with Opus 4.8`}
              </button>
            ) : null}
            {bridgeUp && (
...
function FixChip({ status }: { status: FixState }) {
  const map: Record<FixPhase, { label: string; color: string; bg: string }> = {
    ...
    fixing:   { label: 'Opus fixing…', color: 'var(--brand-color)', bg: 'color-mix(in srgb, var(--brand-color) 12%, transparent)' },
