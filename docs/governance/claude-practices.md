...
| Subagent spawn depth cap (default lowered 5→1) and concurrency cap (default 20) | `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=2` in `.claude/settings.json` env block, now applied to root + all 7 per-app settings files (root and `apps/pce/admin` were missing until 2026-08-21 remediation) — our subagents are leaf agents today, but this guards Pattern G's nested `Explore` spawns from silently failing as the depth-1 default tightens; concurrency cap (20) needs no override, our heaviest fan-out is ≤5 concurrent | current | 2026-08-21 | Agent SDK TypeScript CHANGELOG 0.3.217; docs/governance/claude-updates/2026-08-20-v2235-2238.md ADOPT-1 |
| `Notification` hook (`agent_completed` / `agent_needs_input` for background-session subagents) | Step 1 (capture-only) actually wired 2026-08-21 (the 2026-08-15 "applied" entry was inaccurate — file + hook block didn't exist on disk until this fix) — `.claude/hooks/notification-capture.py` filters to `agent_completed` only (TS SDK 0.3.233: Notification also fires for permission prompts, so the filter is required to avoid log noise) and logs raw payloads to `docs/governance/notification-payloads.log` for shape confirmation. Step 2 pre-capture spec (TS SDK 0.3.238): `task_started` events carry `is_backgrounded: bool` and `spawn_depth: int` — filter for `is_backgrounded: true` to identify background completions. Step 2 (parse + merge into `subagent-stop.py`/`_telemetry.py`) still held until a real payload is captured — verify field paths against the first captured payload before finalizing | partial | 2026-08-21 | Claude Code CHANGELOG 2.1.224; Agent SDK TypeScript 0.3.224, 0.3.233, 0.3.238; docs/governance/claude-updates/2026-08-20-v2235-2238.md ADOPT-2/ADOPT-3 |
