...
| `apps/pce/admin/components/pce/courses-evaluatees/step-survey-instances.tsx` | ~430 | None — wizard step shell | PCE push wizard two-step split, step 2 (Survey design). Composes vendored DataTable (course-grouped, `groupHeaderSlot` band Select) + DS Badge/Button/LocalBanner. Duplicate skip at offering+role+person grain. Documented 2026-07-24. |
