...
| `apps/pce/admin/components/pce/term-themes-insight.tsx` | ~95 | None — composes `AiInsightCard` (documented PCE hand-roll, row above) | Term-level AI theme aggregation card: sentiment-dot chip list (label + N-courses count) derived from `PceSurvey[]` via shared `lib/pce-themes.ts`. DS `MetricInsight` is a single-statement rail (prop interface, not a component); no DS chip-list insight organism exists (672 exports enumerated, ds-adoption-reviewer 2026-07-08). Per Aarti D14 2026-05-08: AI summaries surface before question-level detail at every aggregation level. Date: 2026-07-08. |
...
| `apps/pce/admin/components/pce/courses-evaluatees/step-scope-courses.tsx` | ~430 | None — wizard step shell | PCE push wizard two-step split, step 1 (Courses & students). Composes vendored DataTable + DS Select/Button/LocalBanner + shared TokenSelect (scope-controls.tsx). No DS step-frame organism exists. Documented 2026-07-24. |
| `apps/pce/admin/components/pce/courses-evaluatees/step-survey-instances.tsx` | ~480 | None — wizard step shell (stacked list) | PCE push wizard step 2 (Survey design), promoted Variant E "Stacked question" (Romit, 2026-07-24): per-course stacked lines (new / already-exists / gap) with a fixed control lane; composes DS Collapsible/ToggleSwitch/Checkbox+CheckboxLabel/Select/Button/LocalBanner + SurveyStatusBadgeOS. Not a DataTable surface — 1–4 lines per course; duplicate soft-warning at offering+role+person grain. |
| `apps/pce/admin/components/pce/courses-evaluatees/add-students-sheet.tsx` (checkbox list) | ~40 | DataTable | Same topology as exxat-prism-sheet above: checkbox-picked student rows inside a Sheet with a pinned "Add N students" footer — DS Sheet composition with a plain list, per the documented pattern. Documented 2026-07-24. |
