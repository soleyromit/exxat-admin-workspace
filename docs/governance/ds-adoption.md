...
| `apps/pce/admin/components/pce/survey-qr-dialog.tsx` | ~185 | ExportDrawer (false positive) | **Not a migration candidate — a rule false positive.** `hand-rolled-export-drawer` fires on (`<Dialog` + a `/download\|export/` keyword + no `ExportDrawer` import). The keyword here is the "Download PNG" button, which saves a **QR code image**, not a dataset. `ExportDrawer`'s surface — file-format selector, column picker, async progress — models a tabular export and has no meaning for a single image; adopting it would be a downgrade, not a migration. The dialog itself is DS throughout (`Dialog`/`Card`/`Button`/`LocalBanner`/`Skeleton`); only the one-line anchor download is bespoke. Date: 2026-07-15. |
