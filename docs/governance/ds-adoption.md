...
| `apps/pce/admin/components/pce/permissions-matrix.tsx` | ~110 | DataTable | Static read-only Course Survey RBAC reference matrix (5 roles × 4 function groups). Cross-tab column model — DataTable has no cross-tab support, no sort/filter needed. Uses raw DS Table primitives per Table docstring ("static doc matrices"). Pattern mirrors `distribute-wizard/step-report-access.tsx` above. Date: 2026-09-01. |
| `apps/pce/admin/components/pce/term-breakdown.tsx` | ~198 | None — composes DS `Button` + tint tokens | Status-bucket rows for the dashboard term card's "Breakdown Mode" (Aug 19 2026 feedback, Cases 4–9): icon medallion + label/value + conditional progress bar + per-bucket action buttons. DS has no status-action-row organism — `DataRowList` is a filterable/virtualized list hub, `MetricItem.progress` is a KPI-strip mini-bar with no action buttons (702 exports enumerated, ds-adoption-reviewer 2026-08-19). Medallion anatomy is `dashboard-home.tsx`'s own established local pattern (8 prior occurrences) — reused, not reinvented. Date: 2026-08-19. |
