...
| 14 | I check the default render but skip empty / loading / error / disabled / focus states. Pages ship with `<DataTable data={[]}>` falling back to "No results match your filters" (wrong copy when source is empty), Dialog forms missing `aria-invalid` despite having validation logic, `opacity-60` on Cards containing `text-muted-foreground` text (drops contrast below WCAG 4.5:1), and clickable divs with no `focus-visible:ring` (keyboard users have no focus indicator). | The "imports are right" signal feels like the only DS conformance signal I need; state-coverage was never a regex-able rule until 2026-05-11. Mock data is rich enough that the default render looks good; I never visit the `/route?empty=1` case. | ⏳ partial 2026-05-11 | Added 5 audit rules to `scripts/ds-adoption-audit.py` (phase-0 WARN): `datatable-no-empty-state`, `dialog-no-error-feedback`, `opacity-60-on-text-parent`, `clickable-without-focus-ring`, `async-fetch-no-skeleton`. Surfaces 6 state-coverage gaps across the workspace. Added Pattern F to `docs/governance/verification-discipline.md`. Shipped `docs/patterns/admin/state-coverage.md` (ADMIN-004) with canonical file:line per state. Subagent `state-review` staged at `/tmp/state-review.md` (needs copy into `.claude/agents/` — write permission to `.claude/` is sandboxed). Still owed: canonical state catalog at `docs/governance/component-state-catalog.md` (parallel agent's deliverable); promotion of each rule from warn → block once workspace hits 0 hits per rule. |
