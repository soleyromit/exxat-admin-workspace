...
| 2026-08-15 | v2222-2226 | PARTIAL | **CORRECTION (2026-08-21):** ADOPT-1 and ADOPT-2 "APPLIED" claims below were false — root + `pce/admin` env block and the entire `notification-capture.py` file/hook were never actually on disk. Both fixed in v2235-2238 (2026-08-21); ADOPT-3 was genuinely applied. Original entry retained for history: Covers CC 2.1.222–2.1.226 + TS/Python SDK mirrors (bridges 2.1.202–2.1.219 gap). ADOPT-1: `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=2` env block — claimed APPLIED to root + all 6 per-app `.claude/settings.json`, actually only 5/7. ADOPT-2: Notification `agent_completed` hook — claimed APPLIED Step 1, file did not exist. ADOPT-3: promote `/code-review` from `deferred` to `current` in claude-practices.md — APPLIED (2026-08-15, confirmed genuine). 5 ALREADY-HAVE, 4 DEFER, rest SKIP (SDK-consumer-only / org-admin / passive bug fixes). Also: standalone `.claude/` conventions audit (hooks/skills/subagents/commands all confirmed current; `crossSessionInbound`/`dialogExpiry`/sandbox credential-masking noted as optional, not adopted — n/a to our `acceptEdits` permission mode). |
...
| 2026-08-20 | v2235-2238 | ACCEPTED | State discrepancy found (v2222-2226's "APPLIED" claims were false on disk) and remediated 2026-08-21. ADOPT-1: `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=2` env block added to root `.claude/settings.json` + `apps/pce/admin/.claude/settings.json` — all 7 settings.json files now confirmed to have it (verified via grep). ADOPT-2: `.claude/hooks/notification-capture.py` created (filters to `agent_completed`, handles TS SDK 0.3.233 permission-prompt noise) + `Notification` hook block wired into root `.claude/settings.json` after `SubagentStop` — smoke-tested with synthetic payloads (agent_completed logged, permission_prompt filtered), test log entry removed so the file is clean for the first real capture. Step 2 (parse) still held pending a live payload. ADOPT-3: `claude-practices.md:95-96` updated with corrected dates + TS SDK 0.3.238 `is_backgrounded`/`spawn_depth` field spec for Step 2. 5 ALREADY-HAVE, 7 DEFER, remainder SKIP (bug fixes, SDK consumer APIs, UX preferences). |
| 2026-09-16 | ui-hallucination-and-viz-comprehension | PROPOSED | ADOPT-1: run /skill-doctor to prune 55-skill/9170-line context bloat causing UI hallucination; ADOPT-2: bashOutputMaxChars: 128000 to prevent DS source truncation; ADOPT-3: create exxat-dataviz skill for Plot/ECharts/heatmap patterns. |
