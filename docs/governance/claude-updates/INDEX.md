...
| 2026-08-15 | v2222-2226 | PARTIAL | Covers CC 2.1.222–2.1.226 + TS/Python SDK mirrors (bridges 2.1.202–2.1.219 gap). ADOPT-1: `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=2` env block — APPLIED to root + all 6 per-app `.claude/settings.json` (pce, pce/admin, exam-management/admin, patient-log, patient-log/admin, portal; `Admin/`/`Student/` are graphify DS mirrors, not product apps — skipped), 2026-08-15. ADOPT-2: Notification `agent_completed` hook — APPLIED Step 1 only (`.claude/hooks/notification-capture.py`, capture-mode, logs to `docs/governance/notification-payloads.log`); Step 2 (parse + merge into subagent-stop.py) still held pending a live payload capture. ADOPT-3: promote `/code-review` from `deferred` to `current` in claude-practices.md — APPLIED (2026-08-15). 5 ALREADY-HAVE, 4 DEFER, rest SKIP (SDK-consumer-only / org-admin / passive bug fixes). Also: standalone `.claude/` conventions audit (hooks/skills/subagents/commands all confirmed current; `crossSessionInbound`/`dialogExpiry`/sandbox credential-masking noted as optional, not adopted — n/a to our `acceptEdits` permission mode). |
