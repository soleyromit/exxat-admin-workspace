...
CLAUDE.md §6 holds the ~15 most-used tokens. The full DS reference (token tables, component APIs, theme system; ~8K tokens) lives in `node tools/ds/source.mjs` (+ globals.css) and is loaded on demand:

| Match | Fires |
|---|---|
| Any DS component name (`Button\|Badge\|Sheet\|DataTable\|Sidebar\|Dropdown\|Dialog\|Tooltip\|Tabs?\|Avatar\|Card\|Popover\|InputGroup\|Field\|Select\|Checkbox\|RadioGroup\|Toggle\|Drawer\|Banner\|Calendar\|Breadcrumb`) | `lazy:ds-reference` — Read `node tools/ds/source.mjs` (+ globals.css) before generating UI code |
