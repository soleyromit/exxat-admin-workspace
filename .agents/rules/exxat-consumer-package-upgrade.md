...
|---|---|---|
| Framework wiring (~36) | **Overwritten**, with a backup | Product identity, routing, scope, shell layout, switcher chrome — has to match the installed package or the app gates the wrong products |
| Seams (~84) | **Created if missing, never overwritten** | One-line re-export shims, slot fills (`app-shell-wiring.tsx`, `panel-bridges.tsx`), `globals.css`, `vite.config.ts` |
...
looked identical to a stale copy. The first `upgrade` after the switch converts
existing drift into declarations, then deletes the baseline; from then on the
consumer declares ownership before editing.
