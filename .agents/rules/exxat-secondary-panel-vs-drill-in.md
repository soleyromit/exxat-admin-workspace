...
<!-- Synced from .agents/rules/exxat-secondary-panel-vs-drill-in.mdc - run npx exxat-ui sync-extras after Cursor rule edits -->
...
- Forget the **Esc / Cmd+[ / Ctrl+[** keyboard close on `SidebarDrillIn` — every drill-in MUST be exit-able by keyboard (see `.agents/rules/exxat-accessibility.md` § "Keyboard scope and escape").
...
For `SecondaryPanel` rows that route to distinct paths, use the shared `nav-active` helpers (`isNavHrefActive` / `resolveActiveNavHref`) per **`.agents/rules/exxat-nav-single-active.md`** — those handle longest-prefix matching across the whole nav tree.

## See also

- **`.agents/rules/exxat-primary-nav-secondary-panel.md`** — SecondaryPanel rule (lifecycle, flyout close vs dismiss, surface elevation).
- **`docs/exxat-ds/library-nav-ia-pattern.md`** — Library three-tier nav + flyout behaviour.
- **`.agents/rules/exxat-accessibility.md`** — § "Keyboard scope and escape" (Esc handler required on every drill-in).
- **`.agents/rules/exxat-nav-single-active.md`** — one active row across primary + secondary + drilled-in nav.
