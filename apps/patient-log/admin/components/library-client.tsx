...
import { LibraryRecordRail } from "@/components/library-record-rail"
...
import { LibraryAccessBridge, LibraryFolderBridge } from "@/components/panel-bridges"
import { KeyMetrics } from "@/components/key-metrics"
import { useSidebar } from "@/components/ui/sidebar"
import { useModKeyLabel, useAltKeyLabel } from "@/hooks/use-mod-key-label"
import type { ResponsiveAction } from "@/components/ui/responsive-action-row"
...
import { libraryKpiMetrics } from "@/lib/mock/library-kpi"
import {
...
/** Module scope so the panel-nav hook's deps don't churn on every render. */
const REOPEN_PANEL_PATHNAMES = [LIBRARY_ALL_PATH]
...
export function LibraryClient() {
  ...
  const { navState, searchParamsKey, pathname, isHubPath, hubBasePath, libraryBasePath } = useSecondaryPanelHubNav({
    ...
    reopenPanelOnPathnames: REOPEN_PANEL_PATHNAMES,
  })
  ...
  const [tabs, setTabs] = usePersistedState<ViewTab[]>(
    `${libraryPersistKey}:listpage:tabs:v1`,
    DEFAULT_TABS,
  )
  const [activeTabId, setActiveTabId] = usePersistedState<string>(
    `${libraryPersistKey}:listpage:active-tab:v1`,
    DEFAULT_TABS[0].id,
  )
  ...
  const handleCommitCreation = React.useCallback((spec: CreatedViewSpec) => {
    ...
    setCreatingViewType(null)
    setCreatingName("")
  }, [libraryPersistKey, setActiveTabId, setTabs])
  ...
  React.useEffect(() => {
    ...
    const apply = () => {
      ...
      if (raw && current.tabIds.has(raw)) {
        setActiveTabId(raw)
      } else if (raw) {
        // Stale hash (tab removed) — fall back to default and strip the fragment.
        setActiveTabId("questions")
        const prefix = libraryQueryPrefixFromSearchString(current.searchParamsKey)
        navigate(`${current.hubBasePath}${prefix}`, { replace: true })
      }
      // No hash: keep persisted activeTabId from localStorage.
    }
    apply()
    window.addEventListener("hashchange", apply)
    return () => window.removeEventListener("hashchange", apply)
    // Re-run on pathname changes (mount/unmount); URL search-param changes are read from the ref.
  }, [isHubPath, isDedicatedSearch, navigate, setActiveTabId])

  // When the URL has no hash but a custom tab is active (restored from storage), sync the hash.
  React.useEffect(() => {
    if (!isHubPath || isDedicatedSearch) return
    if (!LIBRARY_LIBRARY_HUB_PATHS.includes(pathname)) return
    const raw = typeof window !== "undefined" ? window.location.hash.slice(1) : ""
    if (raw || activeTabId === "questions") return
    if (!tabIds.has(activeTabId)) return
    const prefix = libraryQueryPrefixFromSearchString(searchParamsKey)
    navigate(`${hubBasePath}${prefix}#${activeTabId}`, { replace: true })
  }, [
    activeTabId,
    hubBasePath,
    isDedicatedSearch,
  ...
  const onActiveTabChange = React.useCallback(
    ...
    [hubBasePath, isHubPath, isDedicatedSearch, navigate, searchParamsKey, setActiveTabId],
  )
  ...
  // ── Record peek rail ───────────────────────────────────────────────────────
  // The open record lives in the URL so a peek is a link a coordinator can send
  // ("look at this one") and survives a reload. It replaces rather than pushes:
  // peeking down a list would otherwise bury the page the user arrived from
  // under a dozen history entries, and Escape or a click in the hub is the way
  // out, not Back.
  const openRecordId = searchParams.get("record")
  const openRecord = React.useMemo(
    () => (openRecordId ? items.find(item => item.id === openRecordId) ?? null : null),
    [items, openRecordId],
  )
  ...
  // Stepping follows what the hub is showing, not the unfiltered bank, so the
  // rail walks the same order the user is reading. The neighbours are resolved
  // from the table's visible rows after each commit; both ends leave their
  // handler undefined, which is what disables that chevron.
  const [step, setStep] = React.useState<{
    previous: string | null
    next: string | null
    position: string | null
  }>({ previous: null, next: null, position: null })

  React.useEffect(() => {
    if (!openRecordId) {
      setStep({ previous: null, next: null, position: null })
      return
    }
    const rows = tableRef.current?.visibleRows ?? []
    const index = rows.findIndex(row => row.id === openRecordId)
    if (index < 0) {
      setStep({ previous: null, next: null, position: null })
      return
    }
    setStep({
      previous: index > 0 ? rows[index - 1].id : null,
      next: index < rows.length - 1 ? rows[index + 1].id : null,
      position: `${index + 1} of ${rows.length}`,
    })
  }, [openRecordId, items, navState, urlToolbarSearchSync])
  ...
  // `variant: "destructive"`. Shortcuts required — see
  // `exxat-bulk-action-shortcuts.mdc`.
  ...
  const libraryBulkActions = React.useMemo<BulkAction<LibraryItem>[]>(
    ...
    [alt, handleBulkFavorite, handleBulkArchive, mod],
  )

  // Toolbar commands act on the hub, not on a selection, so they sit on the
  // toolbar row rather than in the selection bar.
  //
  // **New question** is the page's filled primary and it lives here rather than
  // in the header, because it is the list's own command: it lands beside Import
  // and one row above the questions it adds to. The header is told to drop it
  // (`newQuestionPlacement="toolbar"`) so the page still has exactly one primary
  // (P3) and the chord is bound exactly once.
  //
  // Export is deliberately absent: the header's More menu already owns it, wired
  // to the real `ExportDrawer`. The archive is a place you go rarely, so it
  // declares `overflow` and stops competing for a row slot.
  const libraryToolbarActions = React.useMemo<ResponsiveAction[]>(
    () => [
      {
        id: "new-question",
        label: "New question",
      ...
      },
      {
        ...
        shortcut: `${mod}${alt}I`,
        onSelect: () => {
          console.info("[Library] import questions (demo only)")
        },
      },
      {
        id: "view-archive",
        ...
        shortcut: `${mod}${alt}R`,
        onSelect: () => {
          console.info("[Library] view archive (demo only)")
        },
      },
    ],
    [alt, handleNewQuestion, mod],
  )
  ...
  const activeViewType = React.useMemo(
    () => tabs.find(t => t.id === activeTabId)?.viewType ?? "table",
    [tabs, activeTabId],
  )
  /** Dashboard view renders KPIs inside the canvas key-metrics card — hide the header strip. */
  const showHeaderMetrics = showMetrics && activeViewType !== "dashboard"
      ...
      {({ collaborators, openInvite }) => (
        <>
            ...
            <ListPageTemplate
              ...
              renderContent={(tab, updateTab) => (
                <LibraryTable
                  ...
                  toolbarActions={libraryToolbarActions}
                  onRowClick={row => setOpenRecord(row.id)}
                  openRowId={openRecordId}
          ...
          <LibraryRecordRail
            ...
            onPrevious={step.previous ? () => setOpenRecord(step.previous) : undefined}
            onNext={step.next ? () => setOpenRecord(step.next) : undefined}
            positionLabel={step.position ?? undefined}
          />
