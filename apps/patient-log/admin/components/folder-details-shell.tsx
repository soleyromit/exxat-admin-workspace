    ...
    <nav aria-label="Path in folder tree" className="flex min-w-0 flex-wrap items-center gap-1 text-xs text-muted-foreground">
      {segments.map((seg, idx) => (
        <React.Fragment key={seg.id}>
          {idx > 0 && <i className="fa-light fa-chevron-right shrink-0 text-xs leading-none opacity-40" aria-hidden="true" />}
          <span
            className={cn(
              "min-w-0 truncate",
...
export function FolderDetailsShell({
  ...
  return (
    <div className="flex flex-col bg-card">
      <header className="sticky top-(--shell-utility-bar-height) z-20 isolate shrink-0 border-b border-border/60 bg-card px-4 pb-4 pt-4">
