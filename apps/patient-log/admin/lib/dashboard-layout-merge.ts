...
export function mergeDashboardLayoutGeneric(
  ...
  const visibleByDefault = new Set(defaults.visible)
  ...
  for (const id of allCardIds) {
    ...
    if (!inVisible.has(id) && visibleByDefault.has(id)) {
      visible = [...visible, id]
      inVisible.add(id)
    }
  }
  order = order.filter(id => knownIds.has(id))
  visible = visible.filter(id => knownIds.has(id))
  return {
