...
import { subscribeToExactStorageKey } from "@exxatdesignux/ui/lib/persisted-state"

const MAX_RECENTS = 12

/** Versioned storage envelope — matches the `{v,d}` shape used by leo-recents. */
type StorageEnvelope = { v: 1; d: string[] }

function parseStored(raw: string | null): string[] {
  if (!raw) return []
  try {
    const parsed = JSON.parse(raw) as unknown
    // Versioned envelope: { v: 1, d: string[] }
    if (
      parsed !== null &&
    ...
    // Legacy: raw string[] — migrated on next write.
...
export interface DedicatedSearchRecentsController {
  ...
  clear: () => void
  /** Same-tab CustomEvent plus shared cross-tab `storage` listener. */
...
export function createDedicatedSearchRecentsController(
  ...
  const record = (query: string): void => {
    ...
    try {
      const envelope: StorageEnvelope = { v: 1, d: deduped }
  ...
  const subscribe = (listener: () => void): (() => void) => {
    ...
    const unsubStorage = subscribeToExactStorageKey(storageKey, () => listener())
    return () => {
  ...
  return { eventName, read, record, clear, subscribe }
}
