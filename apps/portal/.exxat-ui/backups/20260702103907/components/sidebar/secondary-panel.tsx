...
import {
  ...
  isLibraryFocusShellWithNestedNav,
  isLibraryFocusedFlowPath,
...
export function SecondaryPanelProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    // Focus shell (e.g. `/library/new`) — compact scope rail; primary sidebar hidden.
    if (isLibraryFocusShellWithNestedNav(pathname, activePanel)) {
  ...
  const focusShellSupersedesPrimarySidebar = isLibraryFocusShellWithNestedNav(
    pathname,
    activePanel,
  )
