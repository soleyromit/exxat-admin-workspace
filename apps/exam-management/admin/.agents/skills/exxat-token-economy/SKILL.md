...
**Canonical index:** `docs/exxat-ds/INDEX.yaml` — jobs, patterns, rules, references.

**Constitution (always loaded):** `.agents/rules/_constitution.exxat-ds.md` — ten commandments; detail in scoped rules.

**Fast router:** run `node scripts/agent-context-router.mjs <surface>` or use `exxat-surface-router` before opening broader docs. Load `docs/exxat-ds/component-selection-guide.md` only when the surface archetype is unclear.
...
| **Add a board / kanban view** | `placements-board-card.tsx`, `exxat-board-cards.md` | All other rules |
| **Add a KPI strip** | `.agents/skills/exxat-kpi/SKILL.md` | `key-metrics.tsx` source unless editing it |
| **Add overlay (export, properties)** | `.agents/skills/exxat-overlays/SKILL.md` | Full drawer source tree |
| **Sidebar nav / panel / drill-in** | `.agents/skills/exxat-sidebar-nav/SKILL.md` | Full `app-sidebar.tsx` unless editing |
| **Write empty-state / error / button copy** | `docs/exxat-ds/handbook/voice-and-tone.md` | All rules |
| **Theme / token tweak** | `src/globals.css`, `packages/ui/tokens/hooks-index.json` | All pattern docs |
| **Design reference / registry doc** | `exxat-design-reference-hub.md`, `exxat-page-scroll-ownership.md`, `doc-typography.ts` | Skeleton placeholders when live preview exists |
| **Status chip color/icon** | `lib/list-status-badges.ts` | Accessibility rule (already covered by the map) |
| **Bug fix** | `rg` for the symbol, read only the matching file | Everything else |
| **Overflowing tab / breadcrumb row** | `horizontal-scroll-controls.tsx` + `horizontal-scroll-region.tsx`, `exxat-horizontal-scroll.md` | Full `tabs.tsx` / `list-page.tsx` unless editing them |
...
9. **Uploaded image ≠ spec?** — If the user attached a screenshot/mockup: extract **IA only** (labels, routes, fields); map to **`component-selection-guide`** + a **reference hub**; **MUST NOT** pixel-copy or plan "match the screenshot"; **MUST NOT** use **`frontend-design`** to mimic the upload — **`exxat-no-image-pixel-copy.md`** + **`exxat-senior-ux`** win.
...
12. **Correct cell per data point?** — Person → **`AvatarInitials` + name + email**; status → **`StatusCell`** (add `options` + `onChange` when the user may change it); progress / money / rating → named **`table-cells`** import — **`.agents/rules/exxat-table-column-cells.md`**, skill **`exxat-table-column-cells`**.
13. **Single page scroll?** — No `overflow-y-auto` / `max-h-[*vh]` doc wrappers inside **`[data-page-scroll]`** — **`.agents/rules/exxat-page-scroll-ownership.md`**. Tables, charts, drawers are the exceptions.
14. **Design reference reuse?** — Theme preview → **`ButtonSegmentedControl`** (`TokensThemeSwitcher`); doc copy → **`DS_DOC_*`** (`text-sm`); no bespoke doc chrome — **`.agents/rules/exxat-design-reference-hub.md`**.
15. **Tabs only for sections, not modes?** — Theme/chart type/view filter → **`ButtonSegmentedControl`** or **`ViewSegmentedControl`** (hubs) or **`DropdownMenu`**; **`Tabs`** only for record sections + ChartCard chart/trend — **`.agents/rules/exxat-tabs-chrome.md`**.
...
| mode, theme, view type, chart type, filter (2–5 options) | **`ButtonSegmentedControl`** (doc/theme) or **`ViewSegmentedControl`** (hub views) — **not** `Tabs` | `@/components/ui/{button-segmented-control,view-segmented-control}` · **`exxat-tabs-chrome.md`** |
...
**Data point → cell:** full decision table in **`docs/exxat-ds/table-column-cells-pattern.md`** — skill **`exxat-table-column-cells`**. Person columns are the most common mistake (plain text instead of **`AvatarInitials` + name + email**).

Every cell renderer below is exported from **`@/components/data-views`** (re-exported from `components/data-views/table-cells.tsx`). Live catalog: `components/columns-showcase.tsx` at route `/columns`.
...
bespoke work (`exxat-reuse-before-custom.md`).
...
- All 29 `.agents/rules/exxat-*.mdc` at once — §1 already points to the 1-2 that apply.
...
- **Cell patterns catalog** — `components/columns-showcase.tsx` (live at `/columns`)
