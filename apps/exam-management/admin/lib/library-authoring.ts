...
/** Default curriculum metadata for demo drafts (Leo + new-item defaults). */
export const AUTHORING_DEFAULT_CURRICULUM = {
  subjectArea: "Category 1" as AuthoringSubjectArea,
  track: "Track 1" as AuthoringDiscipline,
  phase: "preclinical" as AuthoringPhase,
}

// ─────────────────────────────────────────────────────────────────────────────
// Inspector — general / delivery options (assessment delivery)
// ─────────────────────────────────────────────────────────────────────────────

export type AuthoringGeneralSettingKey =
  | "randomizeOptions"
  | "partialCredit"
  | "caseSensitive"
  | "negativeMarking"
  | "showFeedbackWhenReviewing"
  | "eligibleForRandomDraw"
  | "shuffleMatchingPairs"
  | "shuffleOrderingItems"

export type AuthoringGeneralSettings = Record<AuthoringGeneralSettingKey, boolean>

export const AUTHORING_DEFAULT_GENERAL_SETTINGS: AuthoringGeneralSettings = {
  randomizeOptions: true,
  partialCredit: false,
  caseSensitive: false,
  negativeMarking: false,
  showFeedbackWhenReviewing: true,
  eligibleForRandomDraw: true,
  shuffleMatchingPairs: true,
  shuffleOrderingItems: false,
}

export interface AuthoringInspectorToggleOption {
  key: AuthoringGeneralSettingKey
  label: string
  description: string
  /** Omit to show for every question type. */
  applicableTypes?: readonly AuthoringQuestionType[]
}

/** Toggle rows rendered in the composer inspector (filtered by question type). */
export const AUTHORING_INSPECTOR_GENERAL_TOGGLES: readonly AuthoringInspectorToggleOption[] =
  [
    {
      key: "randomizeOptions",
      label: "Randomize options",
      description:
        "Shuffle answer choices for each respondent. Fixed order when off.",
      applicableTypes: ["mcq_single", "mcq_multiple", "true_false"],
    },
    {
      key: "partialCredit",
      label: "Partial credit",
      description: "Award points for each correct selection on multi-select items.",
      applicableTypes: ["mcq_multiple"],
    },
    {
      key: "negativeMarking",
      label: "Negative marking",
      description: "Deduct points for incorrect selections (single- and multi-select).",
      applicableTypes: ["mcq_single", "mcq_multiple"],
    },
    {
      key: "caseSensitive",
      label: "Case sensitive",
      description: "Short-answer grading must match letter casing exactly.",
      applicableTypes: ["short_answer", "fill_blank"],
    },
    {
      key: "shuffleMatchingPairs",
      label: "Randomize match order",
      description: "Shuffle the right-hand column for each respondent.",
      applicableTypes: ["matching"],
    },
    {
      key: "shuffleOrderingItems",
      label: "Randomize step order",
      description: "Present ordering steps in a shuffled sequence to respondents.",
      applicableTypes: ["ordering"],
    },
    {
      key: "showFeedbackWhenReviewing",
      label: "Show feedback when reviewing",
      description: "Reveal rationale and keyed answers after the attempt is submitted.",
    },
    {
      key: "eligibleForRandomDraw",
      label: "Eligible for random draw",
      description: "Include in pools that pull random items from the question bank.",
    },
  ] as const

export function authoringInspectorToggleVisible(
  option: AuthoringInspectorToggleOption,
  type: AuthoringQuestionType,
): boolean {
  if (!option.applicableTypes) return true
  return option.applicableTypes.includes(type)
}

/** Default point value when an item is placed on an assessment (demo). */
export const AUTHORING_DEFAULT_POINTS = "1"
...
/** Reference row kinds — material students may see while answering. */
export type AuthoringReferenceKind = "citation" | "link" | "image" | "document"

export type AuthoringReference = {
  id: string
  kind: AuthoringReferenceKind
  /** Text citation (kind `citation`). */
  citation: string
  /** URL (kind `link`). */
  url: string
  /** Optional student-facing caption for link / image / document. */
  linkLabel: string
  fileName: string
  /** Local blob URL for image preview in the demo composer. */
  previewUrl: string
}

export const AUTHORING_REFERENCE_KIND_OPTIONS = [
  {
    value: "citation" as const,
    label: "Citation",
    icon: "fa-book",
    description: "Text citation reviewers can verify",
  },
  {
    value: "link" as const,
    label: "Link",
    icon: "fa-link",
    description: "URL students may open while answering",
  },
  {
    value: "image" as const,
    label: "Image",
    icon: "fa-image",
    description: "Figure or photo shown with the question",
  },
  {
    value: "document" as const,
    label: "Document",
    icon: "fa-file-lines",
    description: "PDF or file students may read",
  },
] as const

export function createAuthoringReference(
  id: string,
  kind: AuthoringReferenceKind = "citation",
): AuthoringReference {
  return {
    id,
    kind,
...
/** Minimum question length before option suggestions appear in the composer. */
export const OPTION_SUGGESTION_MIN_LEAD_IN = 12

/**
 * Demo suggestions for empty MCQ options — derived from the question stem.
 * In production this would come from Leo / item-writing models.
 */
export function suggestOptionTextsForLeadIn(
  leadIn: string,
  count: number,
  type: AuthoringQuestionType,
): string[] {
  const stem = leadIn.replace(/\s+/g, " ").trim()
  if (stem.length < OPTION_SUGGESTION_MIN_LEAD_IN || type === "true_false") {
  ...
  const topic =
    stem
      .replace(
        /^(which|what|how|when|where|who)\s+(of the following\s+)?(is|are|best|most likely|the correct)\s+/i,
        "",
      )
      .replace(/\?+$/, "")
      .trim()
      .slice(0, 72) || "this scenario"

  const singlePool = [
    `Follow the guideline-recommended first-line approach for ${topic}`,
    `Order indicated diagnostics before changing management`,
    `Choose supportive care only and defer indicated workup`,
    `Start broad therapy before confirming the diagnosis`,
    `Reassure and discharge without the usual follow-up`,
    `Select the contraindicated intervention for the presentation`,
    `Prioritize comfort measures over standard-of-care steps`,
    `Treat a look-alike diagnosis while skipping confirmatory testing`,
  ]

  const multiPool = [
    `Include evidence-based steps that apply to ${topic}`,
    `Select actions that delay necessary evaluation or treatment`,
    `Include both required monitoring and indicated first-line therapy`,
    `Choose options that omit standard safety checks`,
    `Pick interventions supported by current consensus guidance`,
    `Include a step appropriate only after specialist referral`,
    `Select redundant tests that do not change management`,
    `Mark both patient education and appropriate follow-up planning`,
  ]

  const pool = type === "mcq_multiple" ? multiPool : singlePool

  let h = 0
  for (let i = 0; i < stem.length; i++) h = (h * 31 + stem.charCodeAt(i)) >>> 0

  return Array.from({ length: count }, (_, idx) => pool[(h + idx * 7) % pool.length]!)
}
...
/** Fallback PageHeader `<h1>` before the author types a question prompt. */
export const NEW_QUESTION_HEADER_FALLBACK = "New question"

/** Live composer title — collapses whitespace; falls back when the prompt is empty. */
export function questionComposerHeaderTitle(
  leadIn: string | undefined,
  fallback = NEW_QUESTION_HEADER_FALLBACK,
): string {
  const normalized = (leadIn ?? "").replace(/\s+/g, " ").trim()
...
export type LeoQuestionDraftPatch = {
  ...
  subjectArea?: AuthoringSubjectArea | ""
  track?: AuthoringDiscipline | ""
  phase?: AuthoringPhase | ""
  randomizeOptions?: boolean
  partialCredit?: boolean
  caseSensitive?: boolean
  negativeMarking?: boolean
  showFeedbackWhenReviewing?: boolean
  eligibleForRandomDraw?: boolean
  shuffleMatchingPairs?: boolean
  shuffleOrderingItems?: boolean
  points?: string
  bloom?: string
  cogLevel?: string
  tags?: string[]
...
export function buildMockLeoQuestionDraft(
  ...
  const curriculum = AUTHORING_DEFAULT_CURRICULUM
  const general = AUTHORING_DEFAULT_GENERAL_SETTINGS
  ...
  switch (type) {
    ...
    default:
      return {
        ...
        references: [
          {
            ...createAuthoringReference("ref-leo-1", "citation"),
