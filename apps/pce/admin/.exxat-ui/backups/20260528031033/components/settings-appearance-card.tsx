...
import { validateCustomProductSuffix } from "@/lib/product-routing"
import {
  isProductRefHidden,
  isStartupProductRef,
  productRefKey,
...
const BUILTIN_PRODUCT_LABELS: Record<Exclude<Product, "exxat-custom">, string> = {
  "exxat-prism": "Exxat Prism",
  "exxat-one-schools": "Exxat One — Schools",
  "exxat-one-sites": "Exxat One — Sites",
}
...
function labelForProductRef(
  ...
  return BUILTIN_PRODUCT_LABELS[ref.product as Exclude<Product, "exxat-custom">] ?? ref.product
}
...
export function SettingsAppearanceCard() {
  ...
  const productOptions = React.useMemo((): ProductListOption[] => {
    const builtIns: ProductListOption[] = [
      { value: "exxat-prism", label: "Exxat Prism" },
      { value: "exxat-one-schools", label: "Exxat One — Schools" },
      { value: "exxat-one-sites", label: "Exxat One — Sites" },
    ]
    const customs: ProductListOption[] = customProducts.flatMap((cp, customIndex) =>
      isListedCustomProduct(cp)
      ...
      <div>
        {!mounted ? (
          ...
          <FieldGroup className="gap-8">
            <SettingsFormRow
              label="Products"
              description="Panel tint per product; Exxat wordmark stays pink. Set a default for startup."
            >
              <div
                ...
                {productOptions.map(option => {
                  ...
                  return (
                    <div
                      ...
                      className="grid gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                    >
                      <div className="flex min-w-0 flex-col gap-2">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {isDefault ? (
                            <Tip label="Opens when you start the app or visit /" side="top">
                              <span
                                className="inline-flex shrink-0 items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium uppercase tracking-wide text-foreground"
                            ...
                            <Button
                              ...
                              onClick={() => setStartupProduct(rowRef)}
                            >
                              Set as default
                            </Button>
                          )}
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          {addProductBlockedReason ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              ...
                              panel tint follows your brand color. Add as many custom products as
                              you need.
                            </p>
                          )}
                        </div>
