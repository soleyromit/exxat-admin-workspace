...
function InteractiveIcon({ sz, reduced }: { sz: SZ; reduced: boolean }) {
  ...
  const onIconKeyDown = React.useCallback((e: React.KeyboardEvent<HTMLButtonElement>) => {
  ...
  return (
    <Button
      ...
      aria-label="Ask Leo"
      className={cn(
        "relative inline-flex items-center justify-center shrink-0 p-0 rounded-full",
        sz.root,
      )}
      onMouseDown={onDown}
      onMouseUp={onUp}
