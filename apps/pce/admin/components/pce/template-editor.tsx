...
Generated {importedBanner.sections} section{importedBanner.sections !== 1 ? 's' : ''} · {importedBanner.questions} question{importedBanner.questions !== 1 ? 's' : ''} from {importedBanner.file}. Review and edit below.
          ...
          {isProgrammatic ? (
            ...
            /* Guided variant — vertical tabs over the template's evaluations.
               Jul 23 "quiet tabs" pass: rows are plain names with a left
               active bar (the vertical form of the DS line-tabs underline);
               progress is a trailing ✓+count; amber appears only for a role
               set the admin created and left role-less. Adding a role set
               lives on the Faculty stage, never inside the tab list. */
              ...
              >
                {(() => {
                  ...
                  const renderRailItem = (stop: typeof builderStops[number], label: string, opts?: { amber?: boolean; icon?: string; tooltip?: string; plusCount?: number }) => {
                    ...
                    // "+N" compresses the roster; the tooltip carries the full
                    // list of roles this evaluation covers (Romit, Jul 23).
                    return opts?.tooltip ? (
                      <Tooltip key={stop.key}>
                        <TooltipTrigger asChild>{item}</TooltipTrigger>
                        <TooltipContent side="right">{opts.tooltip}</TooltipContent>
                      </Tooltip>
                    ) : item
                  }
                  ...
                  return (
                    <>
                      ...
                      {/* Faculty peer row — the main button enters the aspect
                          ...
                          a hidden active child when the group is folded. */}
                      <div className="flex items-center">
                        <Button
                          variant="ghost"
                          size="sm"
