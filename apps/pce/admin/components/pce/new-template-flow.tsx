...
export function NewTemplateFlow({ embedded = false, onCreated }: {
  ...
  // Embedded (push wizard): hand the id back to the host; standalone: go to the builder route.
  const finish = (id: string) => {
    if (onCreated) onCreated(id)
    else router.push(isGeneral ? `/templates/programmatic/${id}` : `/templates/${id}`)
  }
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      {step === 'import-upload' && (
        <>
          <StepShell centered>
            <header className="mb-8 text-center">
              <h1 className="text-2xl font-normal mb-2" style={{ fontFamily: 'var(--font-heading)' }}>
            ...
            {!fileName ? (
              <Button variant="outline" aria-label="Upload file"
                className="w-full h-auto rounded-2xl border-2 border-dashed flex flex-col items-center gap-3 transition-colors hover:bg-muted"
