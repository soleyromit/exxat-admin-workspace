...
export function SecondaryPanelProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    if (activePanel === "library" || activePanel === "learning-activities") {
      // Record detail needs the icon rail (more canvas). Leaving the primary
      // open from a compact-secondary hub visit would keep labels expanded.
      closePanel({
        mainSidebar: isLearningActivitiesCourseDetailPath(pathname)
          ? "collapse"
          : "leave",
      })
    }
    ...
    >
      ...
      {flyout ? (
        <div className="flex items-center justify-between gap-2 px-2">
          <h2 className="m-0 min-w-0 py-0.5 text-xl font-semibold leading-[1.35] text-sidebar-foreground font-heading">
            {title}
          </h2>
        ...
        <>
          <h2 className="m-0 min-w-0 flex-1 py-0.5 text-xl font-semibold leading-[1.35] text-sidebar-foreground font-heading">
            {title}
          </h2>
