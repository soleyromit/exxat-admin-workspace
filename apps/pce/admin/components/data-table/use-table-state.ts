...
export function useTableState<TData extends Record<string, unknown>>(
  ...
  // ── Group by ──────────────────────────────────────────────────────────────
  const [groupBy, setGroupBy] = React.useState<string | null>(defaultGroupBy ?? null)
  ...
  const groupedRows = React.useMemo(() => {
    ...
    /* If `groupOrder` is supplied, honor it; rows whose keys aren't listed go
       at the end sorted alphabetically. Otherwise fall back to alpha sort. */
    const entries = [...groups.entries()]
    const ordered = groupOrder
      ? [
          ...groupOrder
            .map(k => [k, groups.get(k)] as const)
            .filter((tuple): tuple is readonly [string, TData[]] => Array.isArray(tuple[1])),
          ...entries
