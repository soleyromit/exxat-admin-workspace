 ...
 *
 * Focus workflow routes hide **both** primary sidebar and secondary panel
 * (`lib/focus-workflow.ts`). `SidebarAutoCollapse` collapses primary on mount.
 *
 * The template owns:
    ...
    <>
      ...
      >
        <div className={cn(hasActions ? "min-w-0" : "")}>
          <h1
            className="font-heading mb-2 text-4xl font-semibold tracking-tight leading-none text-foreground"
                     >
            {title}
          </h1>
          {subtitle ? (
          ...
          {!isFinal ? (
            ...
            <Button type="submit" disabled={submitting} aria-busy={submitting}>
              {submitting ? (
                ...
                <>
                  <i className={`fa-light ${submitIcon} text-xs`} aria-hidden="true" />
                  {submitLabel}
                  <KbdGroup className="ms-1.5">
                    <Kbd variant="bare">{mod}⏎</Kbd>
                  </KbdGroup>
...
function FormInspectorBody(props: FormInspectorVariantProps) {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden pb-1">
          {children}
        </div>
        ...
        >
          ...
          <div
            className={cn(
              "flex h-full min-h-0 flex-1 flex-col overflow-hidden pb-1",
...
function defaultContentClass(variant: NewFocusTemplateProps["variant"]): string {
  switch (variant) {
    case "form-inspector":
      return "flex min-h-0 flex-1 flex-col gap-0 overflow-hidden px-3 pt-3 pb-1"
    case "workflow":
    case "shell":
    default:
