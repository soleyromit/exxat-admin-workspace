...
**View tabs overflow:** The views toolbar is wrapped in **`HorizontalScrollRegion`** with **`controlsLayout="group-end"`** (segmented `[← | →]` after the tab bar). Record **`TabsList`** measures itself and sheds labels, then tabs, into an overflow menu with no wrapper. SiteHeader breadcrumbs use the same primitive with **`alignEnd`**. **Do not** hand-build flanking chevrons — **`HorizontalScrollControls`** / **`useHorizontalScrollAffordances`** from `@exxatdesignux/ui/components/ui/horizontal-scroll-controls`. **`docs/horizontal-scroll-pattern.md`**, **`exxat-horizontal-scroll.mdc`**.
