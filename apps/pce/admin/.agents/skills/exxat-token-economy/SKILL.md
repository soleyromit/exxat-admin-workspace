...
12. **Correct cell per data point?** — Person → **`AvatarInitials` + name + email**; status → **`ListHubStatusBadge`**; progress / money / rating → named **`table-cells`** import — **`.agents/rules/exxat-table-column-cells.md`**, skill **`exxat-table-column-cells`**.
