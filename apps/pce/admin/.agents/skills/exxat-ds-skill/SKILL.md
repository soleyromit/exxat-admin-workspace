...
| **Nav row labels** | **`SidebarNavLabel`** on every primary / secondary sidebar row; labels **wrap** (**`whitespace-normal`**, **`break-words`**, **`h-auto`**) — **MUST NOT** **`truncate`** nav copy. Icon rail hides labels; **`tooltip`** exposes the full string. See **`exxat-sidebar-nav-labels.md`**. |
...
**Reference:** `components/sidebar/app-sidebar.tsx` (`NavLinkItems`, `SidebarDrillInItems`), `components/library-secondary-nav.tsx`, **`exxat-sidebar-shell.md`**.
...
**`MetricItem.trend`** must match the **signed change** (arrow direction = truth). **`trendPolarity`** (`higher_is_better` default, **`lower_is_better`**, **`informational`**) controls **tints** and **`aria-label`** — e.g. **low PBI / review flags** rising → `trend: "up"` + **`lower_is_better`** → unfavourable (red), not green. **Doc:** **`docs/kpi-trend-pattern.md`** · **Rule:** **`.agents/rules/exxat-kpi-trends.md`** · **Skill:** **`.agents/skills/exxat-kpi-trends/SKILL.md`**.

### 5.3 KPI count (max four)

**`ListPageTemplate`** metrics and **Data tab** key-metrics cards: **≤ 4** `MetricItem` — **`docs/kpi-strip-max-four-pattern.md`**, **`lib/dashboard-layout-merge.ts`**, **`.agents/rules/exxat-kpi-max-four.md`**, **`.agents/skills/exxat-kpi-max-four/SKILL.md`**.

### 5.4 Cards vs table rows

Dense comparable hub → **`DataTable`**. Boards / folders / visual browse → **`ListPageBoardCard`** + **`ListPageViewFrame`**. **Doc:** **`docs/card-vs-rows-pattern.md`** · **Rule:** **`.agents/rules/exxat-card-vs-list-rows.md`**.
...
| Success / error feedback | Inline copy, `LocalBanner`, or dialog — **never** `toast()` / Sonner / snackbars (**`AGENTS.md` §6.5**, **`.agents/rules/exxat-no-toast.md`**) |
...
Before adding any component: search `components/ui/` first. Add a prop/variant to an existing component rather than creating a parallel one. **If nothing fits** and you need a **new shared primitive or large bespoke widget**, **ask the user** before new files — **`.agents/rules/exxat-reuse-before-custom.md`** (unless the task already approved greenfield).
...
Units belong in `FormDescription` (e.g. GPA → "Out of 4.0"; Hours/week → "hrs/wk"), not hidden in the placeholder. Prefer picker primitives (`DatePickerField`, `Select`) over free-text wherever one exists. Full checklist: `.agents/skills/exxat-accessibility/SKILL.md` — *Form fields — format hints*.
