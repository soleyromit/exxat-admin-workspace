...
8. **Library — folder URL scope** — When **`?scope=folder`**, **`LibraryPageHeader`** **⋯** also lists **Customize folder**; **`LibraryNewFolderSheet`** on **`LibraryClient`** — **`.agents/rules/exxat-library-hub-header.md`**, **`docs/library-hub-header-pattern.md`**.

## MUST NOT

- A second invite control beside a populated face rail.
- Per-hub access label forks or parallel collaborator types.
- Toast/snackbar for invite outcomes (**`exxat-no-toast.md`**).
