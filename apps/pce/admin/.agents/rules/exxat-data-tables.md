...
10. **Header cells advertise what is applied to their column.** `DataTable` marks a column with a small filter / group / conditional-rule icon whenever `activeFilters`, `groupBy`, or a conditional rule with active values names its key, so a user who narrowed a column from the Properties drawer can see which one without reopening it. These are **derived state** — **MUST NOT** hand-render a second badge from a `header:` renderer, and **MUST NOT** track "is filtered" per column in page state. Each icon carries `role="img"` + `aria-label` + a `Tip`; the pinned header clone renders the same icons at `tabIndex={-1}`.
11. **A grouped, selectable table gives every group its own checkbox.** The divider bar leads with a tri-state `Checkbox` that takes or releases only that group's rows through **`useTableState().setRowsSelected(ids, next)`**. Group selection is a **subset** operation: **MUST NOT** reuse `toggleAll` / `setSelected` for it, which replace the whole selection and silently drop batches the user already took from other groups. Derive ids with the **group-local index** (`getRowId(row, i)`) so they match the ids the body rows register under. The pinned group clone mirrors the box non-interactively (`tabIndex={-1}`, `pointer-events-none`) — the real bar stays the keyboard and screen-reader target. The box sits in a slot the width of the **select column**, centred, so it shares an axis with the row checkboxes; do not re-tune it with bar padding.
12. **A group divider speaks the grouped column's language.** `DataTable` looks the group's raw value up in that column's **`filter.options`** and renders the matching **`options[].node`** (the same chip the cells paint) instead of the raw field, and tints the band with **`options[].tone`** (`StatusBadgeTone`, blended toward `--dt-group-bg`). Tone is **declared, not inferred** — the table cannot read a colour back out of arbitrary JSX — so a status-like column MUST ship **`tone`** on every option beside its `node`, from the same map its `tintClassName` comes from. **MUST NOT** pass a pre-rendered chip as the group label from page state, and **MUST NOT** hex a group background. A value with **no** `tone` — or with **`neutral`**, which is the absence of a signal — is **not** left grey: the table hashes the value into the categorical **`--chart-*`** ramp, so non-semantic groupings (term, owner, brand) and quiet statuses (Draft, Not started) still head each batch with a stable colour. The chip in the rows keeps its own neutral grey; only the band takes a hue. With no option at all, the divider falls back to the grouped column's **`cellKind`** — `person` / `people-rail` renders **`AvatarInitials` + name**, other kinds prefix the column's own glyph, and an empty group reads **`None`** with no glyph (there is no identity to rebuild). **MUST NOT** re-render either the tint or the identity from a `header:` / page-state helper; and because both fills are inline styles, forced-colors drops **`[data-dt-group-bar]`** to `Canvas` — new group chrome MUST carry that attribute.
