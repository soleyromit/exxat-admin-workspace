...
2. **Default layout is grouped** — segmented `[← | →]` **after** the viewport (`controlsLayout="group-end"` on `HorizontalScrollRegion`, or `layout="group"` on `HorizontalScrollControls`). Reference: `list-page.tsx` views toolbar, `tabs.tsx` `TabsList`, `page-breadcrumb-trail.tsx`.
...
6. **Let the primitive fade the edge.** `HorizontalScrollViewport` sets `data-fade` from its own scroll position and one rule in `globals.css` masks whichever side has content behind it to transparent, so a clipped row never ends mid-card. It is automatic; do not add a gradient overlay, a `::after` scrim, or a background-coloured fade of your own. Those pick one surface colour and these rows sit on cards, tinted washes, and the page canvas.
...
- `packages/ui/src/components/ui/tabs.tsx` — `TabsList` (builds its own region)
