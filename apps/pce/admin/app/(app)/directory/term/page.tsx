...
import {
  ...
  type ProgramTerm, type TermSeason,
    ...
    },
    {
      ...
      cell: (row) => (
        <RowActions
          ...
          label={row.name}
          actions={[
            // Activate for evaluation lives here (term-level action) — replaces the old
            // per-term toggle and the dashboard's "Activate Term" button.
            ...(row.status !== 'archived'
              ? [{ label: 'Activate for evaluation', icon: 'fa-circle-play', onClick: () => router.push(`/surveys/push?term=${row.id}`) }]
              : []),
