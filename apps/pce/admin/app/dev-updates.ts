...
async function fetchUpdates(product = 'pce', period = 'week', type = 'all'): Promise<Entry[]> {
  try {
...
async function showUpdates(productOrFilter = 'pce', period = 'week', type = 'all') {
  ...
  if (['today', 'week', 'month', 'all'].includes(productOrFilter)) { p = 'pce'; per = productOrFilter }
  ...
  console.group(`%c Exxat Updates · PCE · ${per} (${entries.length})`, 'font-weight:bold;color:#6366f1')
...
export function initDevUpdates() {
  ...
  fetchUpdates('pce', 'today').then(todayEntries => {
