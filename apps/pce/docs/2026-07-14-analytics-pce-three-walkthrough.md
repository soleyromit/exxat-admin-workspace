...
**Date:** 2026-07-14 · **Author:** Claude (browser walkthrough) · **Status:** Archaeology / reference only

---

## ⚠️ Read this first — provenance

**The surface documented here is not built from `apps/pce/admin`, and is not current PCE.**
...
`pce-three` serves an **earlier, diverged generation** of the product. The current deploy at
`exxat-pce-design.vercel.app/analytics` is a different design entirely (3 tabs, Term/Cohort toggle,
Leo AI-insight block, Prism branding, Johns Hopkins tenant). This matches the saved note
*"PCE prod = exxat-pce-design.vercel.app, NOT pce-three"*.

**Use this doc to mine intent from the older IA — not as a description of what PCE is today.**
