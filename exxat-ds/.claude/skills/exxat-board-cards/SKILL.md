...
**Canonical MUST/MUST NOT:** **`exxat-ds/AGENTS.md` §4.4** (or **`./AGENTS.md`** when the workspace is the `exxat-ds` app folder only). **Cursor:** **`.cursor/skills/exxat-board-cards/SKILL.md`**. **Claude (app-only workspace):** **`exxat-ds/.claude/skills/exxat-board-cards/SKILL.md`** — same body. This skill is the **how-to**; the handbook stays the contract.
