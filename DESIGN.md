...
- **DS-018** — Brand presence. Every admin `page.tsx` MUST reference at least one brand token (`--brand-color`, `--brand-tint`, `--brand-color-dark`, `--brand-color-light`, `--brand-color-deep`, `--brand-foreground`, `--sidebar`, `--sidebar-accent`, `--sidebar-primary`). A page that uses only neutral tokens (`--muted`, `--foreground`, `--border`) is DS-conformant code but feels disconnected from product brand. *Gate:* `scripts/brand-presence-audit.py` in pre-commit (warning-only initially; tighten to `--strict` once existing pages are remediated). *Rationale:* the bug visible 2026-05-10 — `/my-surveys/[id]/results` empty state used `--muted` for the lock icon background. DS-conformant; brandless. Brand-present version uses `--brand-tint` background + `--brand-color-dark` icon. Same components, drastically different feel.
- **DS-019** — Brand assignment governance. Every admin app's brand class on `<html>` (`theme-one` / `theme-prism`) MUST be documented in an ADR at `docs/decisions/<NNN>-brand-assignment.md`. New products inherit workspace default (`theme-one`) unless ADR says otherwise. Cross-product brand claims in any spec MUST be verified against the actual `app/layout.tsx` of the cited product before being acted on. *Gate:* code review on `app/layout.tsx` changes; `architecture-audit.py` checks that every admin app's brand class is declared in an ADR. *Rationale:* PCE was switched from `theme-prism` to `theme-one` in April 2026 based on a spec claiming "same as exam-management" — but exam-mgmt is `theme-prism`. The false-premise claim was never verified. Result: undocumented per-product inconsistency.
